<script lang="ts">
    ...
    import { loadAccidents, type AccidentRecord } from '$lib/accidents';
    import { getSafestRoute } from '$lib/routing';
  
    // Map State
    let mapElement: HTMLElement;
    let L: any;
    let mapInstance: Map;
    let accidentsList: AccidentRecord[] = $state([]);
    ...
    async function fetchAccidents() {
        try {
            accidentsList = await loadAccidents();
            plotHeatmapOrPoints();
        } catch (e) {
    ...
    async function getRoute() {
        ...
        routeResult = null;
        try {
            const records = await loadAccidents();
            const data = getSafestRoute(startLat, startLon, endLat, endLon, records);
            routeResult = data;
            drawRoutes(data);
        } catch (e) {
            console.error(e);
            alert("Error computing route. Please try again.");
        } finally {
            isLoading = false;
        }
    }
